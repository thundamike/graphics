 ...
 * @property {number} [size=1]
 */
export class GrPlane extends GrObject {
  constructor(params = {}) {
  ...
  stepWorld(delta, timeOfDay) {
    ...
    let t = this.time;
    let x = 0.05 * Math.cos(t*Math.PI);
    let y = 0.03 * Math.sin(t*Math.PI);
    let z = 0.05 * Math.sin(t*Math.PI);
    // in this animation, use the sine of the accumulated angle to set current rotation.
    // This means the swing moves faster as it reaches the bottom of a swing,
    // and faster at either end of the swing, like a pendulum should.

    this.plane.rotateY(-delta/1150)
    this.plane.position.x = 3 + 4*Math.cos(t/4*Math.PI);
    this.plane.position.z = 3 + 5*Math.sin(t/4*Math.PI);
...
export class GrCarousel extends GrObject {
  ...
  constructor(params = {}) {
    ...
    let opole;
    let horse;
    let horseGeom = new T.BoxGeometry(0.5,0.5,0.5)
    let horseMat = new T.MeshStandardMaterial({color:"orange", metalness:0.5})
    let num_poles = 10;
    let poles = [];
    let horses = [];
  ...
  stepWorld(delta, timeOfDay) {
    ...
    this.horses.forEach(function(horse) {
      horse.position.y = i%2==0 ? 1.6 + 0.5*Math.sin(Math.PI*t) : 1.6 + 0.5*Math.cos(Math.PI*t);
      ++i;
    });
